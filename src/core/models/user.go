...
// User model represents a user in the system, with all required fields.
type User struct {
	...
	Username              string    `gorm:"column:username;type:text;unique;not null" json:"username"` // New field for username
	...
	LocationName          string    `gorm:"-" json:"location_name"` // Used for input only, not persisted in the DB
	LocationID            uuid.UUID `gorm:"column:location_id;type:uuid;not null" json:"location_id"`
	EducationLevelName    string    `gorm:"-" json:"education_level_name"` // Input field for education level name
	EducationLevelID      uuid.UUID `gorm:"column:education_level_id;type:uuid;not null" json:"education_level_id"`
	FieldOfStudyName      string    `gorm:"-" json:"field_of_study_name"` // Input field for field of study name
	FieldOfStudyID        uuid.UUID `gorm:"column:field_of_study_id;type:uuid;not null" json:"field_of_study_id"`
	CollegeName           string    `gorm:"-" json:"college_name"` // Input field for college name
	...
	AuthID                uuid.UUID `gorm:"column:auth_id;type:uuid;unique" json:"auth_id"` // Foreign Key to Auth model
